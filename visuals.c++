...
SDL_Rect inputRect = {(200), 0, INPUT_WIDTH, INPUT_HEIGHT};
...
void renderMenu_digital()
{
    ...
    if (isMenuVisible)
    {
        for (int i = 0; i < numMenuItems; i++)
        {
            ...
            SDL_Rect itemRect = {0, 40 + i * MENU_HEIGHT, MENU_WIDTH, MENU_HEIGHT};
            ...
            SDL_Rect textRect = {0, 45 + i * MENU_HEIGHT, textSurface->w, textSurface->h};
...
{
    SDL_SetRenderDrawColor(gRenderer, 0, 0, 0, 255);
    SDL_Rect boxRect = {0, 10, MAIN_BOX_WIDTH, MAIN_BOX_HEIGHT};
    ...
    if (selectedMenuItem)
    {
        ...
        SDL_Texture *textTexture = SDL_CreateTextureFromSurface(gRenderer, textSurface);
        SDL_Rect textRect = {0, 16, textSurface->w, textSurface->h};
        SDL_RenderCopy(gRenderer, textTexture, nullptr, &textRect);
        SDL_FreeSurface(textSurface);
        SDL_DestroyTexture(textTexture);
    }
