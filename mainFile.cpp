...
void radio_btn(SDL_Renderer *renderer, const char *text, int x, int y, bool selected)
{
...
void render_radio_buttons(SDL_Renderer *renderer)
{
    ...
    radio_btn(renderer, "Analog", 885, 10, selectedSignal == ANALOG);
    radio_btn(renderer, "Digital", 885, 40, selectedSignal == DIGITAL);
}
...
int main(int argc, char *args[])
{
    ...
    // Load font
    font = TTF_OpenFont("./Poppins-Regular.ttf", 16); // Replace with the path to your TTF font file
    if (font == NULL)
    {
    ...
    while (!quit)
    {
        ...
        while (SDL_PollEvent(&event) != 0)
        {
            ...
            else if (event.type == SDL_MOUSEBUTTONDOWN)
            {
                ...
                if (mouseX >= 885 && mouseX <= 905 && mouseY >= 10 && mouseY <= 30)
                {
                    selectedSignal = ANALOG;
                }
                else if (mouseX >= 885 && mouseX <= 905 && mouseY >= 40 && mouseY <= 60)
                {
                ...
                else if (mouseX >= 0 && mouseX <= 0 + MENU_WIDTH &&
                ...
                else if (isMenuVisible)
                {
                    ...
                    {
                        if (mouseX >= 0 && mouseX <= 0 + MENU_WIDTH &&
        ...
        // Clear the screen
        SDL_SetRenderDrawColor(gRenderer, 255, 253, 208, 255);
        ...
        else
        {
            ...
            {
            case 0:
                Non_Return_To_Zero_Level(arr, n, gRenderer, scale);

                break;
            case 1:
                Non_Return_To_Zero_Inverted(arr, n, gRenderer, scale);
                break;
            case 2:
                Manchaster(arr, n, gRenderer, scale);
                break;
            case 3:
                diff_manchester(arr, n, gRenderer, scale);
                break;
            case 4:
                AlternateMarkInversion(arr, n, gRenderer, scale);
                break;
            case 5:
                input_hdb3(arr, n);
                Scramble_AMI(arr, n, gRenderer, scale);
                break;
            case 6:
                input_b8zs(arr, n);
                Scramble_AMI(arr, n, gRenderer, scale);
